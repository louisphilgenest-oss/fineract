...
class CreateFloatingRateCommandHandlerTest {
    ...
    @InjectMocks
    private CreateFloatingRateCommandHandler handler;
    ...
    void testProcessCommandDelegatesToService() {
        ...
        when(writePlatformService.createFloatingRate(command)).thenReturn(expectedResult);

        CommandProcessingResult result = handler.processCommand(command);

        assertEquals(expectedResult, result);
        verify(writePlatformService).createFloatingRate(command);
    }
}
